...
Example usage::
    ...
    pwm = PWM(pin, freq=50, duty=75)  # create a PWM object on a pin
                                            # and set freq and duty
    
    pwm.duty_u16(32768)     # alternative way to set duty to 50%
...
.. class:: PWM(dest, *, freq, duty, duty_u16, duty_ns)
   ...
   Construct and return a new PWM object using the following parameters:
      ...
      - *freq* should be an integer which sets the frequency in Hz for the
        PWM cycle.
      - *duty* should be an integer which sets the duty cycle in percent (0 to 100).
      - *duty_u16* sets the duty cycle as a ratio ``duty_u16 / 65535``.
      - *duty_ns* sets the pulse width in nanoseconds.

   Setting *freq* may affect other PWM objects if the objects share the same
   underlying PWM generator (this is hardware specific).
   Only one of *duty*, *duty_u16* and *duty_ns* should be specified at a time.

Methods
-------

.. method:: PWM.init(*, freq, duty, duty_u16, duty_ns)
...
.. method:: PWM.duty([value])
   ...
   Get or set the current duty cycle of the PWM output, as integer percentage in the range 0 to 100 inclusive.

   With no arguments the percentage duty cycle is returned.
...
* Not all frequencies can be generated with absolute accuracy due to
  ...
  The RP2040 / RP2350 uses a fractional divider, which allow a finer
